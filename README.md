...
**全新 1.2.5 版本** 新增 **管理员 GUI 控制面板**、**基于 PlaceholderAPI 的自动称号系统**（属性插件支持）、**实时语言切换** 以及 **独立功能开关**，让管理更高效、体验更流畅。
...
- ✅ **管理员 GUI 控制面板**（1.2.5 新增）：通过 `/titleadmin panel` 一键查看服务器状态、切换语言、开关功能、重载配置，无需敲命令
- ✅ **自动称号系统（属性插件支持）**（1.2.5 新增）：基于 PlaceholderAPI 占位符，根据玩家等级、金币、击杀数等任意属性自动授予或移除称号，支持 `>=`、`<=`、`==`、`!=`、`>`、`<` 运算符，灵活配置
- ✅ **实时语言切换**（1.2.5 新增）：管理员可在 GUI 面板中一键切换中英文，所有消息即时生效，无需重启
- ✅ **独立功能开关**（1.2.5 新增）：独立控制 RGB 渐变色、内置商店、属性插件支持，按需启用，默认全部关闭
...
- ⚠️ **从 1.2.3 升级到 1.2.4 时，强烈建议删除 `.\plugins\CatCraftTitle\config.yml` 文件**，让插件生成包含新配置项的干净配置文件，否则新功能可能无法正常使用或出现未知 BUG。玩家数据库不受影响，无需删除或迁移。
